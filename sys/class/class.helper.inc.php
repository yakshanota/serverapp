...

{
...

    {
      ...
      public function phone_verify($username)

    {
        ...
        $stmt = $this->db->prepare("SELECT id FROM users WHERE phone = (:username) LIMIT 1");
    ...
    static function sendSms($toMobile,$sendMessage)

    {

        $url="https://www.sms4india.com/api/v1/sendCampaign";
        $message = urlencode($sendMessage);// urlencode your message
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_POST, 1);// set post data to true
        curl_setopt($curl, CURLOPT_POSTFIELDS, "apikey=QRZHUWH0TL7ZQJ5GMC7P4H9Y0FQU6JQ9&secret=428PNQ3OFDXXA3K9&usetype=stage&phone=$toMobile&senderid=SMSIND&message=$message");// post data
